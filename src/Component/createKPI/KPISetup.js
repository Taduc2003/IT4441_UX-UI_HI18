...
// ... các hàm xử lý sự kiện
  ...
  const handleSaveTask = (subGoal, index) => {
    setTasks(prevTasks => ({
      ...prevTasks,
      [subGoal]: prevTasks[subGoal].map((task, i) => i === index ? editedTask : task)
    }));
    setEditingTask(null);
  };
  ...
  const calculateWeight = (subGoal, taskDuration) => {
    const expectedHours = subGoal === 'Giảng dạy' ? 30 : subGoal === 'Nghiên cứu' ? 8 : 4;
  ...
  const [newTask, setNewTask] = useState({          // State để lưu thông tin nhiệm vụ mới
    name: '',
    time: '',
    location: '',
    duration: '',
      ...
      {subGoals.map((subGoal) => (
        ...
        <details open={subGoal === "Giảng dạy"}>
          ...
          <div className="task-list">
            ...
            <button className="add-task-btn" onClick={() => handleShowModal(subGoal)}>+ Thêm nhiệm vụ</button>
            </div>
              ...
              <div key={index} className="task-item">
                ...
                editingTask.index === index ? (
                  ...
                  <>
                    ...
                    />
                    <button className='btn-save' onClick={() => handleSaveTask(subGoal, index)}>
                      Lưu
                    </button>
                    <button className='btn-nope' onClick={handleCancelEdit}>Hủy</button>
                  </>
                ) : (
                  ...
                  <>
                    ...
                    <div className="task-duration">Đơn vị: {task.duration}</div>
                    <div className="task-weight">Trọng số: {calculateWeight(subGoal, task.duration)}%</div>

                    <div className="task-actions">
                      <button className="btn-edit" onClick={() => handleEditTask(subGoal, index)}>Sửa</button>
                      <button className="btn-delete" onClick={() => handleDeleteTask(subGoal, index)}>Xóa</button>
                    </div>
                  </>
                )}
              </div>
  ...
  <Modal.Header closeButton>
    <Modal.Title>Thêm nhiệm vụ mới cho {currentSubGoal}</Modal.Title>
  </Modal.Header>
  <Modal.Body>
    <Form>
      <Form.Group controlId="taskName"> {/* Thêm controlId để kiểm soát form tốt hơn */}
        <Form.Label>Tên nhiệm vụ</Form.Label>
        <Form.Control type="text" value={newTask.name} onChange={(e) => handleTaskInputChange('name', e.target.value)} />
      </Form.Group>

      <Form.Group controlId="taskTime">
        <Form.Label>Thời gian</Form.Label>
        <Form.Control type="text" value={newTask.time} onChange={(e) => handleTaskInputChange('time', e.target.value)} />
      </Form.Group>

      <Form.Group controlId="taskLocation">
        <Form.Label>Địa điểm</Form.Label>
        <Form.Control type="text" value={newTask.location} onChange={(e) => handleTaskInputChange('location', e.target.value)} />
      </Form.Group>

      <Form.Group controlId="taskDuration">
        <Form.Label>Đơn vị</Form.Label>
        <Form.Control type="text" value={newTask.duration} onChange={(e) => handleTaskInputChange('duration', e.target.value)} />
      </Form.Group>

      <Form.Group controlId="taskWeight">
        <Form.Label>Trọng số (%)</Form.Label>
        <Form.Control type="number" value={newTask.weight} onChange={(e) => handleTaskInputChange('weight', e.target.value)} />
      </Form.Group>
    </Form>
  </Modal.Body>
  <Modal.Footer>
